...
describe('Fallback Mechanisms', () => {
  ...
  describe('IPC Handler Availability Checks', () => {
    it('should detect when IPC handlers are not available', async () => {
      ...
      const testHandlerAvailability = async (handlerName: string) => {
        try {
          await mockApi.invoke(handlerName, {});
          return true;
        } catch (error) {
          if ((error as Error).message.includes('Invalid IPC channel')) {
            return false;
          }
          throw error;
        }
  ...
  describe('Folder Selection Fallback', () => {
    it('should fallback to original folder selection when enhanced system fails', async () => {
      ...
      let folderSelectedCallback: Function | null = null;
      mockApi.receive.mockImplementation((channel: string, callback: Function) => {
      ...
      mockApi.send.mockImplementation((channel: string) => {
      ...
      const testFolderSelectionWithFallback = async () => {
        try {
          ...
          return new Promise((resolve, reject) => {
            ...
            cleanup = mockApi.receive('folder-selected', (folderPath: string, imageCount: number) => {
              cleanup();
              resolve({ folderPath, imageCount, source: 'fallback' });
            });

            // Set up listener for errors
            const errorCleanup = mockApi.receive('folder-error', (error: string) => {
              cleanup();
              errorCleanup();
    ...
    it('should provide user-friendly error messages when all systems fail', async () => {
      ...
      const testCompleteFallbackWithUserFeedback = async () => {
        try {
          ...
          try {
            ...
            mockNotificationSystem.showError(
              'Unable to select folder. Please try restarting the application or contact support.'
            );
      ...
      expect(mockNotificationSystem.showWarning).toHaveBeenCalledWith(
        'Primary folder selection failed, trying alternative method...'
      );
  ...
  describe('CSV Upload Fallback', () => {
    it('should fallback to original CSV upload when enhanced system fails', async () => {
      ...
      let csvLoadedCallback: Function | null = null;
      mockApi.receive.mockImplementation((channel: string, callback: Function) => {
      ...
      const testCsvUploadWithFallback = async () => {
        try {
          ...
          return new Promise((resolve, reject) => {
            const cleanup = mockApi.receive('csv-loaded', (csvData: any[], filename: string, entries: number) => {
              cleanup();
              resolve({ csvData, filename, entries, source: 'fallback' });
            });

            const errorCleanup = mockApi.receive('csv-error', (error: string) => {
              cleanup();
              errorCleanup();
              reject(new Error(error));
            });
  ...
  describe('Graceful Degradation', () => {
    it('should disable enhanced features when handlers unavailable but keep basic functionality', async () => {
      ...
      const testFeatureAvailability = async () => {
        ...
        const availabilityResults = await Promise.allSettled(
          enhancedHandlers.map(handler => mockApi.invoke(handler))
        );
    ...
    it('should show appropriate UI feedback when features are degraded', async () => {
      const mockUI = {
        showDegradedModeNotification: jest.fn(),
        hideEnhancedFeatures: jest.fn(),
        showBasicFeatures: jest.fn()
      };
      ...
      const testDegradedModeHandling = async () => {
        try {
          ...
          mockUI.showDegradedModeNotification(
            'Some advanced features are not available. Basic functionality remains enabled.'
          );
      ...
      expect(mockUI.showDegradedModeNotification).toHaveBeenCalledWith(
        'Some advanced features are not available. Basic functionality remains enabled.'
      );
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            ...
            if (attempt === maxRetries) {
              throw new Error(`All ${maxRetries} attempts failed: ${(error as Error).message}`);
            }
            // Wait before retry
        ...
        try {
          ...
          try {
            ...
            mockLogger.error('All folder selection methods failed', {
              enhancedError: (enhancedError as Error).message,
  ...
  describe('System Integration', () => {
    it('should coordinate between enhanced and original systems without conflicts', async () => {
      const mockSystemCoordinator = {
        ...
        activate: jest.fn((system: 'enhanced' | 'original') => {
          if (system === 'enhanced') {
    ...
    it('should prevent simultaneous activation of conflicting systems', async () => {
      const mockSystemGuard = {
        activeSystem: null as 'enhanced' | 'original' | null,
        pendingOperations: new Set<string>(),
        
        canActivate: function(system: 'enhanced' | 'original', operation: string) {
        ...
        activate: function(system: 'enhanced' | 'original', operation: string) {
        ...
        deactivate: function(operation: string) {
