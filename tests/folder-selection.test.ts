...
describe('Folder Selection Integration', () => {
  ...
  describe('Fallback System Integration', () => {
    it('should fallback to original system when enhanced system fails', async () => {
      ...
      const mockOriginalFolderSelection = jest.fn().mockImplementation(() => {
        ...
        setTimeout(() => {
          const mockReceiver = window.api.receive as jest.Mock;
    ...
    it('should provide user-friendly error messages when both systems fail', async () => {
      ...
      expect(mockShowErrorNotification).toHaveBeenCalledWith(
        'Unable to select folder. Please try again or restart the application.'
      );
    });
  });
      ...
      mockEnhancedFileBrowser.selectFolder.mockImplementation(async () => {
        ...
        if (!dialogResult.canceled) {
          ...
          try {
            ...
            throw new Error(`Unable to access folder: ${(error as Error).message}`);
          }
        }
        return null;
      });
