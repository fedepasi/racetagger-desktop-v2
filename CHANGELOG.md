...
### ✨ Review gallery: clearer "Fast keys" hint + Shift+Enter steps back from anywhere

- **The "Fast keys" hint above the recognition results now describes every shortcut accurately and completely.** The old legend was both unclear ("type a number correct" read as broken grammar; "← → skip" actually means *previous/next photo*, not "skip") and incomplete — it never mentioned the keys that shipped with the keyboard-only review (**↑/↓** to move between the numbers on a photo, the trailing empty slot to add one) or the candidate picker (**1–9** to pick a suggested match, **0** for none). It now spells out: `type` sets the number (a digit **replaces** the current value), `Space` **edits** the one that's there, `Enter` confirms & next photo, `← →` previous/next photo, `↑ ↓` move between numbers on this photo, `Esc` close — plus two notes clarifying the type-vs-Space distinction, the empty-slot "add", and the match-suggestion picker. Renderer copy only (`log-visualizer.js`).
- **`Shift+Enter` now steps back one photo regardless of where the cursor is.** Previously it only worked while focused in the Race Number field; from any other field (Drivers/Team) or with no field focused it fell through to "confirm & advance", so stepping back mid-correction forced the mouse. It now always navigates to the previous photo (the per-field Enter listener already ignores `Shift+Enter`, so there's no double save-and-advance; in-progress edits are still saved on leave). `log-visualizer.js` only — no token logic, schema or Edge Function changes.
