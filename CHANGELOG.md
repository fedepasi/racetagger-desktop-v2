...
### 🐛 Folder organization: numberless participants (Team Principal / VIP) now reach their own folder

- **A participant added to a preset without a race number — a team principal, a VIP, a mechanic — now lands in the custom folder you gave them, instead of `Unknown_Numbers`.** Folder organization was entirely number-keyed: a participant identified only by name (and face) had no number to route on, so even after you manually assigned the right name in review, the "Organize into folders" pass dropped those photos into `Unknown_Numbers` — and a scene-skipped frame of that person ended up in a stray `Others` folder (issue #281). The organize pass now also indexes every participant **by name**, so a name-only correction (or a numberless face/AI match) resolves the full preset entry — including its custom `folders[]` and bound PC path — and routes there. When the resolved participant has a custom folder, the photo no longer falls back to `Unknown_Numbers`; a numbered car and a numberless VIP in the same frame both get the photo. A scene-skipped frame you've since tagged with a name or number is treated as identified content and flows through normal organization rather than being buried in `Others`. The organize loop also now processes frames that exist **only** as a user correction (e.g. you assigned a name to a photo the AI left empty), which a plain pass over the AI vehicles would have skipped. Main-process routing only (`main.ts`) — no schema, token logic, or Edge Function changes; covered by `tests/numberless-participant-folder.test.ts`.
...
### ✨ Delivery: Clients are now a filterable table, and you can set a gallery's client from the gallery

- **The "Clients" box on the Delivery page became a proper table — and a gallery can now be assigned to a client from the gallery itself.** Before, clients were a small row of chips that was easy to miss, and there was no way to attach a client when creating or editing a gallery (you could only add galleries from inside a client). Now Clients renders as a **sortable, filterable table** with one row per client showing **avatar + name, type, contact, # galleries, Live (published/total), and total photos**, plus the shareable `/c/…` link (one-click copy). Filter by free text (name / contact / link) or by client type, and click any numeric or text header to sort (numbers default high→low). Each row opens the client detail as before. Two new client pickers close the gap: the **Create gallery** modal has an optional **Client** selector (auto-locked when you create from inside a client), and the **gallery detail** modal has a **Client** dropdown to assign, reassign, or unassign a gallery — no need to enter the client first. Photo totals come from a new read-only `delivery-get-clients-overview` IPC that tallies visible `gallery_images` per gallery in one column-scoped query (no `SELECT *`); the reassignment reuses the existing `delivery-update-gallery` IPC. Renderer + one read-only DB query (`delivery.html`, `delivery-manager.js`, `delivery.css`, `database-service.ts`, `delivery-handlers.ts`, `preload.ts` allowlist) — no schema, token logic or Edge Function changes.

### ✨ "Earn free credits" is now a gamified rewards hub — with an editable referral code

- **The referral page became a full-width rewards hub — your invite link leads as the hero.** At the very top, a bold **amber "Your invite link" hero** puts the thing you actually share first: your invite link big and front-and-center with a one-click **Copy link**, plus your editable **referral code** as a chip (`✎ Edit` — pick something memorable; old links keep working). Below it, your **referral quest** with the progress grouped together — a **milestone track** (`0 → you → 5 → 10 → 25`) with your position lit and the next checkpoint highlighted, a **next-goal banner** with honest math ("3 more friends to a +500 bonus — 300 from referrals plus the 500 bonus, 800 credits in reach"), and a **per-friend reward tower** showing the real scaling tiers (1–5 = 100, 6–15 = 150, 16+ = 200, current band lit). Below that: placeholders for **future quests** (Follow on Instagram, Share a post, Collaborate & earn) and a **Friends joined** table showing who joined, their email, when, and the credits each earned you — with milestone bonus rows (+500 / +1,000 / +2,500) shown as you reach them. The **sidebar** gains a compact "next milestone" badge (`2/5 → +500`). "Earned from referrals" reflects **referral credits only** (per-friend + milestone bonuses), not your whole bonus balance. Milestone math mirrors the real reward logic exactly. Renderer + 2 IPC handlers (`referral.html` / `referral.css` / `referral.js`, sidebar badge in `index.html`, `referral:set-handle` in `survey-handlers.ts` + `preload.ts`); data + the code-edit come from read/write additions to the `satisfaction-survey` Edge Function. **No token logic touched** — the page only displays the existing signup-time referral rewards.
...
- **The sidebar nav was redrawn so non-active items read as buttons, not a flat list.** Before, only the selected item looked interactive; the rest were bare emoji + text with no affordance. Now: **real line icons** (inline SVG) replace the emoji, each in a **visible rounded chip**; rows are roomier with a clear **hover** state (panel background + accent chip); each section now carries its **functional colour** (Participants green, Delivery purple, Earn-credits amber, Home/Analysis blue, Settings neutral) so the nav ties into the app's colour system, and the **active** item is a refined "card" tinted in that section's colour (fill + border + soft glow + icon). Logout is tinted red, and the divider is a clean hairline (dropped the random glowing dot). Calmer overall — removed the old shimmer / slide / glassmorphism per the brand's flat, calm rule. Renderer-only (`index.html` + `css/sidebar.css`) — no logic changes.

### ✨ Review: a "Championship / series sponsors" section right in the Useful Data modal

- **The "Useful Data Found" modal now has a dedicated 🏁 Championship / series sponsors section at the top — so you can stop a sponsor like Pirelli or Castrol from being matched to every car, in one click.** Previously this existed only as a hidden *second* step that appeared **after** you clicked "Update Preset" and a save succeeded — so if you skipped (or never accepted), you never saw it, and series sponsors kept matching every participant. Now RaceTagger detects them **up front**, the moment the modal opens: it counts how many distinct cars in the event carry each sponsor and lists the series-wide ones, **pre-checked when they cover ≥70% of cars** (amber-suggested below that). Tick the ones you want and **Update Preset** adds them to the preset's `series_sponsor_ignore` list — and it works **on its own**, without having to accept any per-participant data. The detection reuses the canonical server-side detector (`detectSeriesSponsors`) via a new dry-run `detect-series-sponsors` IPC (no writes, single source of truth), and candidates already in the preset's ignore list are filtered out. The modal (and the ✨ Improve Preset button) now also appears when there are *only* series sponsors to clean up, even with no per-participant suggestions. Renderer + one read-only IPC (`learned-data-modal.js`, `log-visualizer.js`, `results.html`, `main.ts` handler, `preload.ts` allowlist) — no participant data migration, schema, token logic or Edge Function changes; series sponsors are saved to the existing `participant_presets.series_sponsor_ignore` column (same as the manual "Series sponsors to ignore" editor).
...
### ✨ Satisfaction check-in + invite-a-friend (admin-controlled)

- **After you finish an analysis, RaceTagger may ask a one-question satisfaction check-in — and turn a happy answer into a referral.** When you click **Done** on the results screen and you've analyzed more than a configurable number of photos, a short modal asks how satisfied you are (once, not every time). Rate it well and it offers your **invite link** with the credits you earn per friend who signs up; rate it low and it opens a quick **"what went wrong"** form that flows into the exact same support pipeline as the Support button (it just arrives tagged as coming from a dissatisfied user, so it's triaged with care). Engaged users who haven't referred anyone also get a light "invite a friend" nudge at higher usage milestones. A new **Invite friends** page in the sidebar always shows your referral link, the reward, and your stats. The whole thing is **off by default and fully controlled from the management portal** (enable/disable, thresholds, reward shown, milestones) — no app update needed to turn it on, tune it, or switch it off. **No token logic is touched**: the referral reward is the existing signup-time pipeline; these screens only show it. New `survey-handlers.ts` + `survey-modal` + `referral` page (renderer), a `satisfaction-survey` Edge Function, and one additive `feedback.source` column.

### 🐛 Fix: Support tooltip text was invisible on the Results screen

- **The Support button's tooltip ("Report a bug, request a feature…") was dark-on-dark and unreadable on the Results page.** Its text colour was bound to `--text-primary`, which is white on the main window (where `desktop-theme.css` loads) but dark on `results.html` (which only loads the light `styles.css`), while the tooltip box itself always fell back to a dark fill — so on Results the text vanished into the box. The tooltip is now a **self-contained dark callout** with pinned colours (dark `#1e293b` box, light `#f8fafc` text, muted close button) instead of theme variables, so it renders identically on every page regardless of which stylesheet is in scope. Renderer-only (`feedback-modal.css`) — no participant data, schema, token logic or Edge Function changes.

### ✨ Review gallery: a friendlier "shortcuts" helper (replaces the wall of hint text)

- **The cramped, always-on "Fast keys" line is gone — replaced by a prominent `⌨ Want to review faster? · Discover the keyboard shortcuts` button that opens a clean, centered cheat-sheet.** The old hint was either too terse to understand or, when spelled out, a wall of text in the way. **Click the button or press `?`** and a centered modal opens with the shortcuts grouped and fully legible (keys as JetBrains-Mono keycaps, description right next to each key): move around (`←/→` photos, `↑/↓` detections), edit the number (type / `Space` / `Enter` / `Shift+Enter`), pick a suggestion (`1`–`9`, `0`) — marked with the amber **needs review** badge and a one-line note, since those candidate keys only apply to photos flagged needs-review — and close (`Esc`) — plus a reminder that the always-present empty entry at the bottom is how you add a detection. `Esc`, the ✕, or clicking the backdrop closes it. A centered modal (not an anchored popover) so nothing gets clipped by the narrow controls column. This finally documents the `↑/↓` and `Shift+Enter` shortcuts that the old hint never mentioned. Also makes **`Shift+Enter` go back a photo from any field** (previously only from the Race Number field). Renderer-only (`log-visualizer.js` + `processing-status.css`) — no participant data, schema, token logic or Edge Function changes.
