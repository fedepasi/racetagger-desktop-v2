...
### 🐛 Fix: photos that never got analyzed no longer vanish from the gallery (home count mismatch)

- **The home card sometimes showed more photos than the analysis actually contained** — e.g. "15 photos" on the Recent Analyses card, but opening the analysis showed only 7 or 9, with no explanation (#280). Root cause: on a memory-constrained machine the processing loop can drain or emergency-exit with images still queued. Those photos were counted in the folder total (what the card shows) but never produced a result, so they got no log entry and silently disappeared from the review gallery — while the run still finished and looked "completed". Now, at finalize, any photo that never produced a result is logged as a clearly-marked **"not analyzed"** entry, so it shows up in the gallery as a failed photo you can re-run, the run is honestly marked **completed with errors**, and the counts reconcile. Dropped photos are never charged (they were refunded already, not consumed). For analyses run before this fix, the home card now also explains the gap directly: **"7 of 15 photos analyzed — 8 couldn't be processed · re-run to complete."** Processor + renderer (`unified-image-processor.ts` finalize, `home.js`) — no token logic, schema, or Edge Function changes.
