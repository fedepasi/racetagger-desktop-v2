...
- **The referral page became a full-width rewards hub built around milestones.** Two columns: on the left, your **referral quest** with the progress grouped together — a **milestone track** (`0 → you → 5 → 10 → 25`) with your position lit and the next checkpoint highlighted, a **next-goal banner** with honest math ("3 more friends to a +500 bonus — 300 from referrals plus the 500 bonus, 800 credits in reach"), and a **per-friend reward tower** showing the real scaling tiers (1–5 = 100, 6–15 = 150, 16+ = 200, current band lit). On the right, a dedicated **"Your referral code"** card where you can **edit your code** (`✎ Edit` — pick something memorable; old links keep working) and copy your invite link. Below: placeholders for **future quests** (Follow on Instagram, Share a post, Collaborate & earn) and a **Friends joined** table showing who joined, their email, when, and the credits each earned you — with milestone bonus rows (+500 / +1,000 / +2,500) shown as you reach them. The **sidebar** gains a compact "next milestone" badge (`2/5 → +500`). "Earned from referrals" reflects **referral credits only** (per-friend + milestone bonuses), not your whole bonus balance. Milestone math mirrors the real reward logic exactly. Renderer + 2 IPC handlers (`referral.html` / `referral.css` / `referral.js`, sidebar badge in `index.html`, `referral:set-handle` in `survey-handlers.ts` + `preload.ts`); data + the code-edit come from read/write additions to the `satisfaction-survey` Edge Function. **No token logic touched** — the page only displays the existing signup-time referral rewards.
