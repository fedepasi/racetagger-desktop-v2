...
### 🐛 Fix: an analysis now retries photos it dropped, instead of quietly finishing without them

- **The home card sometimes showed more photos than the analysis actually contained** — e.g. "15 photos" on the Recent Analyses card, but opening the analysis showed only 7 or 9, with no explanation (#280). Root cause: on a memory-constrained machine the processing loop can drain or emergency-exit with images still queued. Those photos were counted in the folder total (what the card shows) but never produced a result, so they got no log entry and silently disappeared from the review gallery — while the run still finished and looked "completed". The goal is to analyze them **all**, so before an analysis closes it now **re-checks for any photo that never got a result and retries it** — one at a time, with the memory freed first (the same memory-recovery mode the batch already falls back to). Recovered photos become normal results in the gallery. Only if a photo still can't be processed after that retry (the machine is genuinely out of memory) is it surfaced as a clearly-marked **"not analyzed"** photo you can re-run, the run is honestly marked **completed with errors**, and the counts reconcile — never charged for the un-analyzed ones (they're refunded, not consumed). For analyses run before this fix, the home card also explains the gap directly: **"7 of 15 photos analyzed — 8 couldn't be processed · re-run to complete."** Processor + renderer (`unified-image-processor.ts` finalize, `home.js`) — no token logic, schema, or Edge Function changes.
