...
-- Rimuoviamo la tabella profiles che non serve più
...
BEGIN
  ...
  -- Tenta update del record esistente nella tabella user_tokens
  UPDATE user_tokens 
...
-- Funzione di fallback per compatibilità con Edge Function esistente
...
AS $$
BEGIN
  -- Delega alla funzione principale che ora usa user_tokens
...
-- Funzione di utilità per ottenere il bilancio token corrente dalla user_tokens
